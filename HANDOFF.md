...
### Q39 — reutilizar parsing de variation de structures
...
**LOAD WORLDS CARD LAYOUT — 2026-09-21:** metadata dos save cards reorganizada em grade fixa de 2×3. Cabeçalho agora contém somente o nome do world. Primeira linha: **Último salvamento / Seed / Dias passados**. Segunda linha: **Dimensão / Bioma / Coordenadas**. Coordenadas permanecem no padrão **X · Z · Y** e agora são o último campo, ao lado de Bioma. Commit funcional: `c516969f7d1c6cd00200c49c8146374114610cef`. **Pendência:** CI do commit em execução. **Próximo passo:** runtime QA visual do espaçamento dos cards; depois continuar auditoria/checkpoint F.

**WORLD LOAD SPAWN + SAVE CARD REFINEMENT — VERSION 0.50.14 — 2026-09-21:** corrigido panic de spawn ao carregar worlds. A causa era o bootstrap filtrar a janela inicial para manter somente chunks já presentes no save; como saves persistem apenas chunks modificados, terreno gerado e intocado ao redor da posição salva podia não existir no `VoxelWorld` restaurado, fazendo `safe_spawn_position` falhar mesmo em coordenadas válidas. O filtro foi removido: chunks persistidos continuam sendo restaurados e chunks intocados ausentes são regenerados deterministicamente usando seed/worldgen version já validados. Commits: `c9b8c058d76ec0cf80e4c9c6b759c0cde25a3fb5` + comentário de invariância `959a1e2b5323d5e0396cc6e34e2201621ffe9e10`; CI passou.

Tela de Load Worlds refinada conforme QA: coordenadas agora seguem **X · Z · Y**; timestamp passou de ISO cru para formato legível por idioma (EN: `Sep 21, 2026 · 14:17 UTC`; PT-BR/ES: `21/09/2026 · 14:17 UTC`) com label localizada “Last saved / Último salvamento / Último guardado”; metadata foi redistribuída em duas linhas equilibradas — linha 1: Seed / Dias Passados / Coordenadas; linha 2: Dimensão / Bioma — com valores em tipografia primária e ações isoladas à direita. Commits: `0916377964fcf7439169d6e7d4ed4f7e90155475` + localizações `4272a7c7`/`a979569d`/`b75bfc72`. CI final do commit funcional `b75bfc72a16f10b3551c43d8ba991310c8dc1914` passou localization audit, Clippy `-D warnings` e `cargo check --locked`. Bump: `f0b16baec16a990ff2da10bb287883417b87d129`. **Pendências:** runtime QA visual da tela e teste manual de load do save que falhava em IVec2(385, -489). **Próximo passo:** continuar auditoria profunda do projeto a partir do checkpoint F.

**HOTFIX / DATA SWEEP + WORLD SELECTION REDESIGN — VERSION 0.50.13 — 2026-09-21:** corrigido segundo ID stale de inventory category após a migração de naming de árvores: `foliage.blockIcon` agora usa `asteria:leaf_oak` (`d166ffce0e63cf05d591922d1a2507e6b80b8dc5`); `wooden_blocks` já havia sido corrigido para `asteria:log_oak` em `891b6e13b467ca48739ea68e8e03ad4ee04557e3`. Foi feita varredura manual pela árvore real do repositório em todos os **74 JSONs data-driven não-localization** procurando os IDs antigos `oak_log/oak_leaf/willow_log/willow_leaf/world_tree_log/world_tree_leaf`; resultado final: **zero ocorrências restantes**. Structures/biomes já estavam usando `log_oak/leaf_oak/log_willow/leaf_willow/log_world_tree/leaf_world_tree` corretamente.

Também corrigida a validação excessiva de tool icon: `icon == ""` é um contrato válido e já possui fallback textual na UI; `asteria:shears` depende disso. Paths de tool continuam validados quando não vazios. Commit: `90290dfba79630d44242c7a62736149c7f659f4d`.

Tela de **Load Worlds** redesenhada: removido o card-pai que continha todos os saves; cada save agora é seu próprio `surface::settings_content()`, com nome e horário, mais **seed, dias passados, dimensão atual, coordenadas e bioma atual**, e ações Load/Delete no próprio card. O empty state de nenhum mundo recuperável virou mensagem centralizada, maior e com mais ênfase. Para suportar o bioma exato sem recomputar heurísticas na UI, snapshots novos persistem `currentBiome` como campo opcional/backward-compatible; saves antigos continuam carregando e exibem “Unavailable/Indisponível/No disponible” até serem salvos novamente. O resumo da seleção agora é construído a partir do snapshot restaurável mais recente. Commits principais: `c4ea338f`, `242b158e`, `ff744d06`, `538a4a2c`, `34d7df9d`, `e41266ab`, `7098109a`, `5099be8d`, localizações `871987be`/`729f50ad`/`915ec659`. CI do commit funcional final (`915ec65964325dd941bff709bc662a59d8975b23`) passou localization audit, Clippy `-D warnings` e `cargo check --locked`. Bump: `cfae851dd801813df9422c83b084ffb2ee27de05`. **Pendências:** runtime QA visual da tela ainda não executado; saves antigos não possuem `currentBiome` até próximo save. **Próximo passo:** continuar auditoria profunda do projeto a partir do checkpoint F registrado anteriormente, usando árvore completa + CI para evitar falsas conclusões sobre dead code/defaults opcionais.

**CORREÇÃO DE AUDITORIA — tool icons — 2026-09-21:** removida a validação excessiva que tornava `ToolDefinition.icon` obrigatório. O contrato existente da UI já possui fallback textual para tools com `icon == ""` (`spawn_tool_icon` e inventory cursor); `asteria:shears` usa esse caminho legitimamente. Agora paths são validados apenas quando `icon` não está vazio; `tintIcon` continua opcional e validado quando presente. Hotfix funcional: `90290dfba79630d44242c7a62736149c7f659f4d`, com teste de regressão para tool sem icon. **Próximo passo:** confirmar CI e continuar redesign da tela de saves.

**CORREÇÃO DE AUDITORIA — block orientations — 2026-09-21:** removida a validação introduzida na etapa 15 que exigia `orientations` não vazio. O contrato existente de `BlockDefinition` é explícito: lista ausente/vazia significa orientação padrão `Y` (`default_orientation()` faz `unwrap_or_default()`; `is_rotatable()` só exige >1). Exigir ao menos uma orientação quebrou blocos válidos como `asteria:bassalt`, `stone` e outros JSONs sem campo `orientations`. Correção funcional: `aa5f3d04687545f2ac38f17ec0cfa2d9de9034d2`, com teste de regressão cobrindo default Y, non-rotatable e `next_orientation` para lista vazia. **Pendência:** CI do hotfix em execução. **Próximo passo:** confirmar CI verde e continuar auditoria sem transformar defaults opcionais em required fields sem checar a semântica dos accessors/defaults.

**AUDITORIA DE MELHORES PRÁTICAS — checkpoint E — VERSION 0.50.11 — 2026-09-21:** bloco de lifecycle/UI/content trust boundaries consolidado. Correções principais: state de creatures/viewmodel/brush isolado por world; mortos deixam de contar em spawn limits e não recebem novos ataques; language dropdown deixou de escrever por frame e shared settings state reseta ao entrar em NewWorld; required tools e JSONs de data agora falham cedo; placeholder multiplicity validada no runtime; asset paths data-driven passam por validação compartilhada; structure anchors e /locate ficaram overflow-safe; save catalog agora valida spawnBiome antes de ativar/salvar um world. O CI revelou e ajudou a corrigir uma remoção incorreta de `VoxelWorld::add_layer_at`; API restaurada e o registro histórico da etapa 14 foi corrigido. Último CI funcional antes do bump: push + PR verdes em `39366cb5e9de90415d4a81bc4d730b49d4e98a56`. Bump: `8395e8d24d3df35da6056d69f800b2503312dc94`. **Pendências:** runtime QA ainda não executado; continuar auditoria de save/session/render lifecycle e revisar remaining `Local<>`/panic boundaries com árvore completa + CI como confirmação. **Próximo passo:** checkpoint F — world/session cleanup, save catalog/pruning invariants, render caches/assets e remaining UI state boundaries.

**AUDITORIA DE MELHORES PRÁTICAS — etapa 29 — 2026-09-21:** `SaveRegistries` agora inclui `BiomeRegistry` e valida `spawn_biome` salvo contra a mesma regra do bootstrap: biome existente, `Surface`, presente na dimension e sem `requireNear`. O catálogo assíncrono carrega a informação necessária para pruning/verification, e publicação de save usa a mesma validação. Commits: `a641071458c00539bc511fccf401b1cd71505389`, `dd58fee079450240fed806c32c96b3afffcffb66`, `56710e29f5088319bd8be7654dafc78ca60d2a86`, `fca9d9e3c7a467493c4e2235be35977332bf060e`. O CI anterior desses commits ainda carregava um teste tautológico de locate; corrigido em `39366cb5e9de90415d4a81bc4d730b49d4e98a56`, que passou push e PR.

**AUDITORIA DE MELHORES PRÁTICAS — etapa 27 — 2026-09-21:** localization/runtime e asset paths foram alinhados aos boundaries do CI/modding. O runtime agora compara multiplicidade de placeholders, não apenas o conjunto de nomes, com teste de regressão (`cae87b023a3e4b3a4b9e8e6a8f3249a5923e19cf`, `c78cb9858e853d4455519feca3c5a76bea74f187`), igual ao checker Python que já usa `Counter`. Foi criado `content::asset_path::is_safe_relative_asset_path` e aplicado a block/layer/tool/sky/player, com creature reutilizando a base e mantendo roots/ext específicos (`930645fc`, `db78a33e`, `ed0f2063`, `090b6473`, `b9e199d2`, `f743fb80`, `2615e8d6`, `394fbe5d`). Paths vazios, absolutos, traversal, backslash, drive/colon e NUL agora falham no load.

**AUDITORIA DE MELHORES PRÁTICAS — etapa 28 — 2026-09-21:** arithmetic de structures/chat endurecida. `candidate_anchor` agora usa checked arithmetic para `cell * spacing + halfSpacing + jitter`, retornando `None` fora do domínio i32, com teste de regressão (`db172f279ef9fc1041942e1529f1ca2761dd9622`). `/locate` agora pula ring candidates fora do domínio, valida chunk→block multiplication, protege probe offsets e usa distance-squared saturating (`cf2dca2ba64989d36caf3abff8d03e8b92e8365e`). O CI também apontou `clippy::too_many_arguments` na otimização do language dropdown; corrigido sem suppression usando `Ref<Interaction>::is_changed()` (`375f1ca4590a3bfa2ab60bb3ea92e1f0531289ba`).

**CORREÇÃO DE AUDITORIA / CI — 2026-09-21:** o CI de `develop` revelou que a etapa 14 havia removido uma API ativa de placement de layers. `VoxelWorld::add_layer_at` é chamado por `src/voxel/edit.rs`; foi restaurado em `ef563f2f541197180e7329078c7b7cb758fc84e5`. O registro histórico da etapa 14 abaixo foi corrigido para não preservar a conclusão falsa. A partir deste ponto, remoções de dead code devem ser confirmadas pela árvore completa e pelo CI.
...
**AUDITORIA DE MELHORES PRÁTICAS — etapa 14 — CORRIGIDA — 2026-09-21:** a tentativa original de remover `VoxelWorld::add_layer_at` foi incorreta: a busca de código usada naquele passe não indexou o caller real em `src/voxel/edit.rs`, e o CI confirmou que `VoxelTopologyRuntime::add_layer` depende dessa API, além dos testes de world. Os commits `02ae9f487185eb6bb756df4c5848c501ea8d2595` e `de81f369a50428611c15702e879f433fa252783a` devem ser tratados como revertidos logicamente. A API e imports foram restaurados em `ef563f2f541197180e7329078c7b7cb758fc84e5`, mas o `#[allow(dead_code)]` não voltou porque a função é de fato usada e não precisa de suppression. **Lição aplicada:** para dead-code removal, confirmar callers pela árvore/compilação, não apenas pelo endpoint de busca.
...
**ESTADO AUTORITATIVO ATUAL — 2026-09-21:** `develop`, Rust + Bevy 0.19.1, `VERSION 0.50.6`. Layers passaram a ser itens completos de inventário/creative: `LayerDefinition` agora possui `name` e `category`, IDs de layer são persistidos/restaurados no backpack/hotbar e aparecem no catálogo, cursor e hotbar. `asteria:moss` agora se chama exatamente **Moss** e pertence a `natural_blocks`; ao selecionar uma layer no creative, botão direito aplica a layer diretamente à face atingida do bloco. Nova tool `asteria:shears`, sem textura própria por enquanto: botão direito remove somente a **surface layer mais externa da face atingida**, usando a ordem autoritativa do canal de layers; layers internas da própria textura do bloco (base/overlay como grass overlay) não fazem parte desse canal e portanto nunca são removidas pela tesoura. A mutação de remoção persiste o chunk e agenda remesh sem alterar collision/raycast do bloco. A HUD de bloco alvo agora mostra as surface layers externas aplicadas ao voxel, deduplicadas por nome, e atualiza imediatamente após placement/removal; texture layers internas continuam omitidas. Save validation/pruning e world activation reconhecem layers como inventory items. Checkpoints funcionais: feature `4e338eeae78fa1add876ab1d3ec16b3a633afdc3`, refactor de activation registries `67fb3765cf26661ab6b833fd08b224e682b528b1`. CI da feature `35552075926` e CI final da release `35552157849` no checkpoint `6dd656ae37c6d6cc9415241e57c1059a6387444d`: **success** em auditoria de localizações, Clippy rigoroso e `cargo check --locked`. Não houve `cargo run`, `cargo test` nem QA visual/runtime Windows; validar in-game placement de Moss, remoção de stacks de layers com Shears, ícones/text fallback no creative/hotbar e atualização da target HUD.

**ESTADO AUTORITATIVO ATUAL — 2026-09-21:** `develop`, Rust + Bevy 0.19.1, `VERSION 0.50.5`. Folhagem da `asteria:world_tree` reconstruída novamente para corrigir o topo sem canopy e copas de galhos escassas/regulares. O esqueleto autoritativo de madeira/pedra foi preservado exatamente voxel por voxel em coordenadas relativas ao anchor; somente as folhas e o envelope vazio do asset mudaram. A folhagem passou de **18.669 para 31.792 folhas**. As 26 regiões externas de término dos galhos agora recebem múltiplos subclusters sobrepostos, assimétricos e com borda/negative-space ruidosos; a cobertura média medida ao redor dessas terminações subiu de ~699 para ~900 folhas. O topo deixou de ter um trecho central sem folhagem seguido por um blob isolado: a nova copa começa a envolver o tronco superior por volta de Y=77 e forma vários lóbulos independentes/overlapping até Y=104, com skyline e vazios irregulares. Pequenos componentes de folhas sem conexão com madeira foram removidos. Para impedir novo truncamento horizontal, o canvas foi ampliado de 95x106 para 111x122 e o anchor deslocado de (47,58) para (55,66), preservando todas as posições relativas; a folhagem final mantém pelo menos 8 blocos vazios até qualquer borda. Checkpoint funcional `b7b2d67f7ed03c911bd670f8ef0c09317d79ac85`; release `0.50.5` em `9c459bc8ee0ebada969ccac1489edd216feeb66c`; CI da feature `35551022154` e CI final da release `35551101495`: **success** em auditoria, Clippy rigoroso e `cargo check --locked`. Não houve `cargo run` nem QA visual/runtime Windows. Validar em chunks novos, pois World Trees já materializadas/salvas não são regeneradas automaticamente.

**ESTADO AUTORITATIVO ATUAL — 2026-09-21:** `develop`, Rust + Bevy 0.19.1, `VERSION 0.50.4`. Sistema data-driven de **voxel surface layers** concluído. Layers são um canal esparso independente de block/fluid, anexado a faces de um voxel-suporte, com até 16 layers por voxel, múltiplas na mesma face, rotação de textura, offset, tint, alpha cutoff/blend e shadow flag; não entram em collision/raycast nem tornam o voxel sólido. Storage/arquivamento/save-load persistem layers, removem-nas automaticamente quando o block ID de suporte muda e mantêm saves antigos legíveis via campo opcional no chunk. O meshing gera quads apenas nas faces expostas, participa do remesh assíncrono e reutiliza iluminação/material de terrain. Structures ganharam `palette.*.surfaceLayers` com `layer`, `faces` e `chance`; o sorteio é determinístico por world seed + structure + layer + posição + face e também é respeitado por `/place`. Conteúdo novo: `asteria:bricks`, `asteria:clay`, `asteria:snow`, `asteria:willow_log`, `asteria:willow_leaf` e layer `asteria:moss`; o bloco `asteria:lamp` já existente passou a usar naturalmente o novo asset `lamp/base.png`. `tree_willow_01..03` agora usam willow_log/willow_leaf; apenas o símbolo vertical `W` do tronco recebe moss nas quatro faces laterais, chance 0.22 por face, com rotação determinística. Checkpoints: core `1b46864f` → persist/render `fa6ffea0` → lint fix `d99e9ab` → content/surfaceLayers `8459bf1f` → `/place` `6962f38b` → import fix `e30ce4f2`. CI da feature `35550372333` e CI final da release `35550450017` no checkpoint `d681ccb17c7479d2f9bb0f8c82ce33881ccebd88`: **success** em auditoria de localizações, Clippy rigoroso e `cargo check --locked`. Não houve `cargo run`, `cargo test` nem QA visual/runtime Windows; validar visualmente os novos blocos, alpha do moss/folhas, densidade dos patches de moss e willows em chunks novos. Chunks/structures já materializados no mundo atual não são reescritos retroativamente.

**ESTADO AUTORITATIVO ATUAL — 2026-09-21:** `develop`, Rust + Bevy 0.19.1, `VERSION 0.50.3`. Ajuste visual de árvores no checkpoint funcional `7be6f105471e79f3f0288791bdf9f53f0d774202`. Investigação do truncamento restante da World Tree confirmou que o resolver/rasterização horizontal por chunk usa os bounds completos da structure; o problema remanescente estava no próprio asset: 23 terminações externas de madeira na copa (raio ~25–39 blocos) acabavam abruptamente dentro do layout, criando aparência de galho serrado mesmo com margem vazia no JSON. Essas terminações foram prolongadas 3–5 blocos, ganharam pequenos forks laterais e depois receberam massas de folhagem em torno das novas pontas. A folhagem da World Tree foi reconstruída preservando tronco/raízes/galhos: deixou de ser um maciço oval contínuo (~26k folhas) e passou para ~15k folhas em clusters menores, irregulares, com recortes/negative space e skyline desigual; nenhuma célula ocupada toca a borda horizontal do novo canvas. As `tree_oak_01..04` e `tree_willow_01..03` também tiveram apenas a folhagem refeita sobre os esqueletos de madeira existentes: oaks usam massas independentes por ponta de galho + recortes e assimetrias; willows usam umbrellas quebrados + cortinas pendentes. Dimensões ocupadas após o ajuste: oak01 10x8x8, oak02 9x11x10, oak03 13x8x12, oak04 9x10x10; willow01 11x9x11, willow02 11x11x9, willow03 11x10x12. CI `35548575543`: **success** em auditoria de dados/localizações, Clippy rigoroso e `cargo check --locked`. Não houve `cargo run`, `cargo test` nem QA visual/runtime Windows; validar em chunks novos porque structures já salvas não regeneram automaticamente.

**ESTADO AUTORITATIVO ATUAL — 2026-09-20:** `develop`, Rust + Bevy 0.19.1, `VERSION 0.50.2`. Feature de ecology/proximity + árvores em `a9d75b611e0e876f6e579270dcd2d0a780e53564`, com correções de import/lint em `34077f3bc57204bbb3b768770ebf2aaf4dbcfcb0` e `a3930a47bcdacb6d1a4c57fafe934f044253fa53`; preservada no HEAD atual `ccec87b85fef8da408283a7d40ef07c442477478`. `StructureRestrictions` agora aceita `proximity` com `mode: "required" | "forbidden"`, alvo contendo exatamente um de `block` ou `fluid`, `minDistance` opcional e `maxDistance` obrigatório. Distância é horizontal X/Z em blocos; target fluid consulta hydrology de superfície + surface fluids authored (ex.: lava de volcano), e target block consulta material de superfície/hydrology, sem depender de chunk já rasterizado. Content validation rejeita IDs inexistentes, target vazio/ambíguo, min>max, regras duplicadas, required+forbidden idênticos e maxDistance > 64. Novo grupo `asteria:tree_willow` em Plains com `tree_willow_01..03.json`, todos usando temporariamente oak_log/oak_leaf e exigindo `asteria:water` a 5–10 blocos do anchor; placement do grupo em Plains: spacing 36, chance 0.72, jitter 11. Dimensões ocupadas: willow 01 7x10x7, 02 9x11x7, 03 9x10x9. As quatro `tree_oak_*` foram redesenhadas para silhuetas inspiradas em open-grown/bur oaks: troncos mais baixos/robustos ou bifurcados, galhos grossos horizontais e copas largas, lobadas e irregulares. Dimensões ocupadas: oak01 9x10x9, oak02 8x12x7 (mais alta/estreita), oak03 11x9x9 (mais baixa/larga), oak04 10x11x9 (assimétrica/forked). Nenhuma das quatro é equivalente a outra por rotações de 90°. CI push `35547661311` no checkpoint funcional `f1e10859ad59b547c876762cbbc3fcabaff8f784`: **success** em auditoria de localizações/dados, Clippy `--locked --all-targets --all-features -- -D warnings` e `cargo check --locked`. Não houve `cargo test`, `cargo run` nem QA visual/runtime Windows. Chunks já gerados não regeneram automaticamente as árvores; QA deve usar chunks/mundo novos.
...
**ESTADO AUTORITATIVO ATUAL — 2026-09-20:** `develop`, Rust + Bevy 0.19.1, `VERSION 0.47.0`. Feature de localização/warp implementada em `676658655aa90f08fe1d55d7a66572491f90c573`, fix de SystemParam em `b77bf54b75ba1dee01ee6632b9fed0483eb52833` e schema final obrigatório em `18e82860c645f90b4907d177c4fc123b4a50d1aa`. O chat agora aceita `/locate biome <id>`, `/locate structure <id>` e `/warp <x> <z> <y>`. Structures exigem `locatable: true|false` no arquivo; World Tree está `true`, oak tree, stone marker e todos os boulders estão `false`. O autocomplete de locate filtra structures não localizáveis. Locate roda em task assíncrona e reutiliza a resolução autoritativa de estruturas do worldgen, incluindo restrições/prioridades/conflitos; biome locate usa o BiomeField. Resultado localizado aparece como `{name} found at X: {x} Z: {z} Y: {y}. [Warp to]`, com `[Warp to]` lilás pelo token `theme::BORDER_FOCUS` e clique roteado pelo mesmo parser de `/warp`. Warp pendente desloca temporariamente o centro do streaming para carregar o destino sem mover o player; só conclui após encontrar posição seca, carregada, com suporte e AABB completo livre usando colisão microblock-aware, então reseta movimento residual. CI push `35544405057` no HEAD funcional `18e82860c645f90b4907d177c4fc123b4a50d1aa`: **success** em auditoria de localizações/dados, Clippy `--locked --all-targets --all-features -- -D warnings` e `cargo check --locked`. Não houve `cargo test`, `cargo run` nem QA visual/runtime Windows. Não existe infraestrutura de mapa/minimap ainda; esta etapa implementa o link clicável no chat, não marker em mapa. **Próximo passo:** QA runtime de locate para World Tree/biomas, clique em Warp to, warp manual para destino sólido e destino distante/unloaded, além de verificar tempo de busca em estruturas/biomas raros.

**ESTADO AUTORITATIVO ATUAL — 2026-09-20:** `develop`, Rust + Bevy 0.19.1, `VERSION 0.46.0`. Checkpoint funcional `449866218ea7094ab8f5ba33dffd9dfc4607a6ed` (`feat: make settings sections scroll together`). As telas de Settings deixaram de trocar sections por `Display::None`: Game Settings mostra continuamente `Graphics → HUD → Languages`; World Settings em gameplay mostra continuamente `World Settings → Game Rules`; New World mostra continuamente `World Settings → World Generation → Game Rules`. Em New World, `World Settings` contém Name/Seed/Game Mode e `World Generation` é exclusivo da criação de mundo, contendo Spawn Biome/Biome Size Multiplier. A sidebar agora funciona como âncora de scroll; o `ScrollPosition` é a autoridade para o botão ativo; espaço de cauda permite a última section chegar ao topo. Mudanças de section por scroll não fecham dropdowns nem resetam inputs. Troca de idioma preserva a section através de scroll pendente após o rebuild. CI push `35542073608`: **success** em auditoria EN/PT-BR/ES, Clippy `--locked --all-targets --all-features -- -D warnings` e `cargo check --locked`. Não houve `cargo test`, `cargo run` nem QA visual/runtime Windows. **Próximo passo:** QA real das três telas, especialmente âncoras/sidebar em scroll manual, último item chegando ao topo, resize 720p/1080p/1440p e dropdowns durante scroll.
