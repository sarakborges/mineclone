...
**Último código:** `.25` `ec38346a97e0d238f184aa32ed0a7c768ef0e3e4` biome selection, bump `87a27fe920a2d44405cb8eb02c4b4563c4f16d1b`; `.26` `d492c3685babc705d8a6e248a2e0a6ebf91decaa` hydrology density SmallVec, bump `25b262065a9a3a995ddd2c6ab427ed0f3f660c55`; `.27` `bbf9947655b04bc80689394352f8a71e632cb1bf` hydrology one-pass sample, bump `7fc04dc45b7fbe43bab796662ec4bb5ad8a96387`; `.28` `2f6e5928553f3ed06bfc9720183346b0273e5e32` neighbor lighting fix, follow-up `9b0e5444e16e3ced3f97514241d44ed9b329427b` restored original retirement assertion, bump `72eb67066781dbe4fb2a7fb8fdfeff373a714966`. **VERSION = 0.15.28.** HEAD may be subsequent docs-only commit: recheck before write. Precedent .24 sky `1d8d096` test `cb5222e`, bump `3b5503e`.

**CI:** versions .3–.21 Clippy + Check success (details history); .22 run35119214356 failed OnceLock syntax, fixed .23; .23 run35120838368 failed dead-code sky mix, fixed .24; .24 run35121054358 Clippy+Check SUCCESS; .25 run35122730177 Clippy+Check SUCCESS (job104884239808); .26 run35123049209 Clippy+Check SUCCESS (job104885298933); **.27 run35123661593 Clippy+Check COMPLETED/SUCCESS (job104887329235). .28 run35124362037 em execução na última consulta, resultado PENDENTE; não chamar aprovado sem consultar jobs.** CI NÃO executa unit tests nem gameplay; evitar afirmar FPS/visual comprovados. Não relatar .22/.23 como aprovados.

## Prioridades — feedback 2026-09-16

| Item | Estado | Próximo passo |
| --- | --- | --- |
| P0.1 FPS/carregamento | Melhorou, ainda aberto; biome .25/hydrology .26/.27 removem alocações/duplicação sem FPS medido; .28 adiciona remesh estritamente borda com conteúdo | Medir durante movimento; throughput/custo Fluid remesh .22 e .28 |
| P0.2 chunks escuros/sombras tardias | Aberto visualmente; seed única .17, Geometry freshness .21, Fluid freshness .22, **notificação halo ao renderizar vizinho vazio/ar .28 (CI pendente)** | Gameplay verificar bordas e sombras, primeira mesh stale e custo de remesh |
...
- .15.25 `ec38346a97e0d238f184aa32ed0a7c768ef0e3e4` bump `87a27fe920a2d44405cb8eb02c4b4563c4f16d1b`: remove Vec oito vizinhos usando `[usize;8]`, remove Vec fallback raw com count+nth, reutiliza único Vec ponderado no fallback climático em vez de três vetores. Mantém hash/critérios/pesos/ordem, teste reuso. CI35122730177 Clippy+Check success. `cargo test` não rodado, FPS/biomas não aferidos.
- .15.26 `d492c3685babc705d8a6e248a2e0a6ebf91decaa` bump `25b262065a9a3a995ddd2c6ab427ed0f3f660c55`: `DensityColumnProfile.water_bodies` usa `SmallVec<[VerticalDensityDelta; 4]>` no lugar de `Vec`; colunas com ≤4 corpos não alocam e >4 usam heap, sem mudar cálculos/margens. Teste seis deltas somam -21. CI35123049209 Clippy+Check success. FPS não aferido.
- .15.27 `bbf9947655b04bc80689394352f8a71e632cb1bf` bump `7fc04dc45b7fbe43bab796662ec4bb5ad8a96387`: `density_column_profile` amostra `WaterBody::normalized_horizontal_distance` uma vez por corpo, reutiliza para strength e shore, substitui scan separado; conserva desempate max_by magnitude e soma carve. Teste compara strength em três posições. **CI35123661593 Clippy+Check success**, runtime não confirmado.
- **.15.28** `2f6e5928553f3ed06bfc9720183346b0273e5e32` + `9b0e5444e16e3ced3f97514241d44ed9b329427b` bump `72eb67066781dbe4fb2a7fb8fdfeff373a714966`: `streaming.rs::notify_loaded_chunk_neighbors` reenqueue Geometry para vizinho renderizado quando sua face tem conteúdo, mesmo se chunk recém-renderizado está vazio ou sua face é ar; necessária porque `mesh_lighting.rs::face_lighting` amostra halo e luz direta inicial pode chegar sem alteração na posterior relaxação. Fluid remesh continua separado se qualquer face contiver fluido. Apenas vizinhos renderizados com conteúdo de borda geram Geometry; teste de aposentadoria anteriormente existente foi preservado (commit follow-up diff apenas 5 linhas). **CI35124362037 em execução na consulta**, validar Clippy/Check e gameplay dark border/FPS antes de encerrar P0.2.

## Subsistemas / investigação direcionada

### Streaming, FPS e iluminação

Preload all-direction+2, corredor frontal+8; fog start~78% end~98% raio conforme readiness; histerese RD4=5/6 RD12=14/16 RD24=26/30; retention R+max(ceil(R/2),10): RD4=14 RD12=22 RD24=36. Geração/remesh async, integração por orçamento. `hydrology/region/density.rs` .26 usa SmallVec inline4 e .27 um único scan/normalização por corpo (sin/cos+noise angular). Cloud mesh .10, lazy water .13, seed única .17, biome .25, hydrology .26/.27 são otimizações localizadas sem FPS medido; .28 acrescenta Geometry remesh na chegada de vizinho ao detectar conteúdo na face existente; medir tempo de remesh em movimento.

Antes .17 `dispatch_initial_mesh_tasks` reseed a cada retry e `VoxelWorld::rebuild_chunk_light` apagava luz convergida. Seed única e unload remove marca. `lighting_updates.rs` 2ms/4096 voxels; remesh 2 tasks/frame/4 inflight/2 integrações. `ChunkMeshDependencies` inicial compara só conteúdo para time-to-visible; `VoxelWorld::chunk_mesh_revisions` inclui luz. .21 Geometry valida halo lighting e não pode sobrescrever Lighting mais nova; .22 separa Terrain/Fluid filas e refresca fluid onde existe água, versionando halo. .28: uma nova região vazia já tem `VoxelLight` direto sem que `relax` altere células; notificação antiga só atualizava fluid vizinho quando nova região vazia. `face_lighting` amostra até nove voxels do halo; atualizar Geometry vizinho cuja face contém conteúdo. **Ainda é possível primeira mesh publicar luz antiga durante processamento assíncrono**, `ChunkMeshTasks` valida só conteúdo. Avaliar snapshot revisão de iluminação/relight posterior preservando time-to-visible e evitando starvation. P0.2/FPS seguem abertos sem gameplay.

### Hidrologia

`generation/density.rs` hydrology density + surface carver; `density_sampling/hydrology.rs::enforce_hydrology_water_volume` abre bed->waterLevel headroom limitado. .14 protege leito/teto fade bounded e water_near lazy. Parede persistindo exige investigar cave connector/túnel/threshold sem mexer margem .12. .26/.27 alteram somente armazenamento e redundância de amostras de corpos d'água, não geometria projetada.
...
1. Confirmar CI `.28` run35124362037; corrigir qualquer erro/aviso concreto. Não confundir Clippy/Check com cargo test/gameplay.
2. P0.2 verificar bordas na chegada de chunk vazio/ar com .28, custo extra Geometry vs sombras escuras e primeira mesh stale; medir FPS/throughput Fluid .22.
