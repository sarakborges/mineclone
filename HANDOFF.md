...
4. Mesh e remesh validam revisões de conteúdo e iluminação separadamente. `ChunkRemeshTasks::lighting_revisions` não é `VoxelWorld::chunk_mesh_revisions`; boundary remesh corrige halo quando vizinho chega.
...
**Último commit de código:** `a1d3fde8f9856e0a56609e1eedab8b4a96aceb56` (`river/selection.rs`: não poluir cache ao atingir trace limit). Bump `754973c2325dc1acb0142a062805c126ede616a9`. `VERSION = 0.15.20`. Commits posteriores de HANDOFF são apenas documentais sem bump. Reconsultar HEAD/VERSION antes de novas mudanças.

**CI Clippy + Check confirmado até 0.15.20:** 0.15.3 run35054771262; 0.15.4 #2064; .5 #2066; .6 #2068; .7 #2070; .8 #2075; .9 #2078; .10 #2080; .11 #2082; .12 #2084; .13 run35109440316; .14 run35111512775; .15 run35112785390; .16 run35112964210; .17 run35113608338; .18 run35114063244; **.19 run35116468021 completed/success, Clippy success e Check success; .20 run35116860460 completed/success, Clippy success e Check success.** Ambos confirmados na mesma sessão após publicação. CI não executa unit tests (`cargo test`) nem valida gameplay.

## Prioridades — feedback 2026-09-16

| Item | Estado | Próximo passo |
| --- | --- | --- |
| P0.1 FPS/carregamento | Melhorou, ainda aberto | Profiling e otimizações mensuráveis; não elevar budgets às cegas |
| P0.2 chunks inteiramente escuros/sombras tardias | Aberto visualmente; fix seed única 0.15.17 CI aprovado | Verificar convergência, halo/revisions, remesh e FPS |
| P0.3 rios cortam túneis com parede reta | Aberto visualmente; patch carver 0.15.14 CI aprovado | Feedback, density/carvers sem regredir margem |
| P0.4 rios sem ligação com lago/oceano | Aberto visualmente; patches 0.15.15/.16/.19/.20 passaram CI | Continuidade física em seed real, cross-region, ocean outlet, confluência |
...
| Bug R distorce held block | 0.15.18 CI aprovado, ainda sem confirmação visual | Pressionar R com bloco orientável e conferir geometria/escala |

Ordem: preservar FPS e P0.5. CI .19/.20 confirmado; validar rio real e luz, depois P0.3, P0.6/7 e R. Não encerrar bug visual com CI apenas.
...
- **0.15.19** `b04ad3f`, `391322c`, `bba6608`, `9044a40`, bump `5fd8b56`: `river/path/curve.rs` usa downstream authoritative `river_height` ou lake_level exato como endpoint; remove queda artificial `min(start - 0.5)`. `river/path/terrain.rs` ajusta APENAS pontos internos, preserva Y de endpoints, não mergulha abaixo da saída para depois subir; remove constante morta. Regressões adicionadas para relief0.1, dois trechos seguidos mesmo X/Z/Y, entrada/saída lago, endpoints e terreno interno, mesmo trecho atravessando fronteira de duas regiões com water height/strength iguais. **CI run35116468021 aprovado (Clippy + Check); `cargo test` não executado, gameplay ainda aberto.** Tradeoff possível: vale abaixo da altura final pode criar seção elevada; investigar por seed antes de novas mudanças.
- **0.15.20** `a1d3fde` bump `754973c`: em `river/selection.rs::drainage_reaches_water_destination`, quando limite RIVER_FLOW_TRACE_STEPS=64 esgota sem destino, resultado negativo é inconclusivo para nós internos; cache grava false somente para origem, evitando envenenar downstream mais próximo de lago/oceano. Se destino ou dead end efetivo confirmados, cache compartilha caminho todo. Helper `cache_reachability`; testes limite não propaga falso e cache de sucesso/beco real completo. **CI run35116860460 aprovado (Clippy + Check); gameplay aberto.** Não confundir trace-limit com prova de desconexão e não desabilitar limite sem profiling.

## Subsistemas / investigação direcionada

### Streaming, FPS, iluminação

Pipeline async acima. Preload all-direction+2, corredor frontal+8 ao andar; fog start~78% end~98% raio conforme readiness; histerese RD4=5/6 RD12=14/16 RD24=26/30; retention R+max(ceil(R/2),10): RD4=14 RD12=22 RD24=36. Geração/remesh async e integração budgetada. `hydrology/region/density.rs` constrói Vec<VerticalDensityDelta> por coluna e testa lagos em carve+shore; só otimizar com evidência e sem mudar geometria. 0.15.10 cloud mesh, 0.15.13 lazy water, 0.15.17 seed única são ganhos localizados; medir FPS parado/andando CPU worldgen/lighting/remesh versus GPU clouds.

Antes .17, `dispatch_initial_mesh_tasks` reseed a cada retry, `VoxelWorld::rebuild_chunk_light` apagava luz convergida e invalidava snapshots. Seed única agora, remove marca no unload. `lighting_updates.rs` budget2ms/4096 voxels; remesh 2 tasks/frame/4 in flight/2 integrações. Seed revision VoxelWorld pode não refletir `ChunkRemeshTasks::lighting_revisions` até dynamic updates. Diagnosticar dados de luz vs mesh escura, halo freshness, sky propagation. P0.2 não resolvido visualmente.

### Hidrologia

`generation/density.rs` compõe hydrology density+surface carver; `density_sampling/hydrology.rs::enforce_hydrology_water_volume` abre água bed->waterLevel e headroom limitado. 0.15.14 protege leito/teto com fade vertical bounded e lazy water_near. Persistindo parede, cave connector/túnel/threshold, sem mexer P0.5.

`river/selection.rs::build_flow_cache` source radius14, target8, trace64; `keep_only_complete_downstream_paths` segue wet ocean/lake. `river.rs::build_river_system` arestas por região com margin4, reachability, valid_lake_outlet. `river/path/confluence.rs` recorta segmentos por bounds+RIVER_MAXIMUM_RADIUS. .15 ancora X/Z, .16 filtra saída seca, .19 ancora Y/testa mesmo edge entre regiões, .20 protege cache contra limite. **Não há teste end-to-end de conectividade física por seed:** testar seleção regional, confluence/lake levels, fonte de água rasterizada, ocean mouth, fluido entre chunks. `DrainageNetwork::is_wet_ocean` calcula piso com node point sample; `HydrologyRegion::water_at` calcula piso com macro 5×5 interpolado; divergência pode produzir outlet declarado molhado e água física seca — HIPÓTESE, validar antes de patch. `river_height` usa threshold continentalness diferente do teste wet floor; conferir transição de altura no mar. Não prolongar rios arbitrariamente, não regredir margens.

### Biomas e cor

`biome_field/selection.rs`: suitability de temperature/humidity/continentalness/erosion, pesos só elegíveis, vizinho dominante/avoidNear 8 vizinhos, fallback raw. `surface.rs`: Voronoi + Mountains belt/peak. `surface_site_spacing` deriva do maior mínimo regional, não raio individual. Plains120–420, Wasteland120–420, Witchwood/Enchanted140–460, Mountains macro .85; pesos Plains1, Wasteland1.30 Witchwood/Enchanted1.25. Wasteland e Plains usam rolling + mesmos surfaceLayers grass/dirt/stone, embora tint/structures diferentes; semelhança visual NÃO prova frequência real. Medir por seed antes de mudar peso. Diagnóstico exploratório do `mountain_belt`: ridge `1 - abs(fractal_noise)` favorece ruído próximo de zero; uma amostragem aproximada de seed42 não substitui contagem do bioma final e não justifica alterar pesos sem medir.

`world/biome.rs` acompanha jogador e combina surface/hydrology/volume em `identity.rs`. `rendering/environment.rs` calcula sky_color e fog_color HSI por CurrentBiome/fase, `rendering/sky.rs` escreve ClearColor. `fog/color.rs`/`fog/attachment.rs` usam sky_color no DistanceFog para esconder silhueta, ignoram fog_color artístico (fato confirmado). Trocar diretamente por fog_color causa diferença terminal com fundo: conceber transição que preserve terminal. Céu sem mudança demanda rastrear identidade e updates antes de mexer em fog.

### Coast, celestial e HUD

Coast identidade usa continentalness raw `BiomeField::climate_at`, oceano físico usa macro interpolado5×5; material rejeita ocean bed acima seaLevel+3, identidade não testa altura. P1 informado concluído, reabrir apenas com evidência. `rendering/celestial.rs` usa CelestialBodyDefinition sun/moon texturizados. Nuvens world-space Y seaLevel relativo/wind desacoplado. P2 informado concluído: hotbar slot vazio oculta held, ghost alpha uniforme, dye, HUD histórica. HUD planejada: player inferior esquerdo Yogg'Sara vida50/100 dentro da barra/status acima; target acima crosshair, bloco esquerda igual slot inventory, nome e infos direita com tooltip shadow. R fix .18 runtime pendente.

---

# Próxima execução

1. CI .19/.20 já confirmado Clippy+Check verde; não repetir consulta sem motivo. `cargo test` não roda no CI. Verificar gameplay das correções de rios .15–.20 em seed e coordenadas reais, em especial conectividade de fluidos entre regiões, lagos e oceano.
2. P0.4: reproduzir end-to-end por seed/coord em regiões vizinhas; amostrar edge Y/fluido por chunk, lake/confluence, ocean outlet. Preservar .12/.15/.16/.19/.20. Possível discrepância entre floor pontual e macro físico é HIPÓTESE até reprodução.
3. P0.2: verificar seed única .17, tracking de revisions, iluminação persistente versus remesh e FPS.
4. P0.3: feedback de tunnel walls após .14, corrigir composição density/carver se necessário sem regredir margem.
5. P0.6: medir seleção real versus semelhança visual e influência mountain belt; P0.7: CurrentBiome->EnvironmentVisualState->ClearColor e fog artística com terminal igual sky.
6. Confirmar R/viewmodel .18 e FPS em gameplay. Não dar como resolvido por CI.
