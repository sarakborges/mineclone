...
**Último código:** `.25` `ec38346a97e0d238f184aa32ed0a7c768ef0e3e4` (`biome_field/selection.rs`), bump `87a27fe920a2d44405cb8eb02c4b4563c4f16d1b`; `.26` `d492c3685babc705d8a6e248a2e0a6ebf91decaa` (`hydrology/region/density.rs`), bump `25b262065a9a3a995ddd2c6ab427ed0f3f660c55`; `.27` `bbf9947655b04bc80689394352f8a71e632cb1bf` (`hydrology/region/density.rs`), bump `7fc04dc45b7fbe43bab796662ec4bb5ad8a96387`. **VERSION = 0.15.27**. HEAD pode ser commit documental posterior: conferir antes de escrever. Precedente .24 `1d8d096` sky e `cb5222e` teste, bump `3b5503e`.

**CI:** versões .3–.21 confirmadas Clippy + Check success (detalhes no histórico); .22 run35119214356 falhou sintaxe `OnceLock<Arc<[Option<...>]>>` corrigida .23; .23 run35120838368 falhou avisos `dead-code` da tentativa de mistura de sky descartada na .24; .24 run35121054358 **sucesso Clippy + Check**. **.25 run35122730177 COMPLETED/SUCCESS Clippy + Check** (job 104884239808). **.26 run35123049209 COMPLETED/SUCCESS Clippy + Check** (job 104885298933). **.27 run35123661593 em fila na primeira consulta; verificar seu resultado**, sem presumir aprovação. CI NÃO executa unit tests/gameplay. Não relatar .22/.23 como aprovados.

## Prioridades — feedback 2026-09-16

| Item | Estado | Próximo passo |
| --- | --- | --- |
| P0.1 FPS/carregamento | Melhorou, ainda aberto; biome .25 e hydrology .26/.27 removem alocações/duplicação sem FPS medido | Medir durante movimento; custo Fluid remesh .22, worldgen/biomes |
...
1. Confirmar CI `.27` run35123661593; corrigir erros/warnings concretos. Não confundir Clippy/Check com cargo test/gameplay.
2. P0.2 primeira mesh vs iluminação revisada, relight posterior sem starvation e sem render duplo desnecessário; conferir streaming neighbor notifications e medir FPS/throughput Fluid .22.
