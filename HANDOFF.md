...
Último HEAD de código publicado: `5c99616c735e95394eb35aebd5fc5399af126801`  
`VERSION = 0.15.7`

CI: 0.15.3 final (`5297a69`) run `35054771262` success; 0.15.4 corrigida (`fbe301f`) run #2064 success; 0.15.5 (`fa84d81`) run #2066 success; 0.15.6 (`f8d244c`) run #2068 success. CI #2070 da 0.15.7 iniciado; checar conclusão antes de publicar o próximo bloco.
...
Investigação estática em 0.15.7: `track_current_biome` pede hydrology overlay usando continentalness direta de `BiomeField::climate_at(horizontal)`; oceano físico em `HydrologyRegion::water_at` usa `macro_sample_at`, grid 5×5 interpolado, e mesma função `ocean_strength` depois da interpolação. Essa diferença de amostragem pode desalinhar identidade Coast da margem de água. Investigar com caso reproduzível e comparar continentalness raw/interpolada, thresholds e coast blend; não reduzir thresholds ou trocar dados sem confirmar a causa. `hydrology_biome_weights` tem bandas: surface/coast até strength 0.25; coast 0.25–0.55; coast/ocean 0.55–0.80; ocean após 0.80.

## P1.3 — Clouds

Causa concreta: `src/rendering/sky_layers/clouds.rs` gera altitude absoluta Y34–48, enquanto Overworld `seaLevel=90`, embora X/Z acompanhem câmera. Corrigir para altitude **relativa ao `sea_level` data-driven da dimensão**, mantendo Y independente da câmera. Fix preparado, mas ainda não publicado neste handoff; validar em runtime nuvens acima da superfície e em dimensões com outro seaLevel.
