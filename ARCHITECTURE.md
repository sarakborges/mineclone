...
- Atomic JSON publication is durable only after the containing directory is synced following the rename. If that post-rename directory sync fails, the storage owner must remove the newly visible file, sync that rollback, and return an error rather than leave an ambiguously committed manifest/snapshot.
