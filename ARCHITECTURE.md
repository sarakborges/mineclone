...
- Complex HUD layouts should keep the composition root separate from semantically distinct view regions. Inventory layout uses `creative`, `player`, and `item` owners: catalog/search/category rendering, player inventory/hotbar/trash rendering, and reusable item/cursor rendering respectively. The parent `layout` module owns only shared view contexts and root composition; sibling sync/interaction modules consume the same narrow reexported capabilities.
...
- World-selection activation uses a prepare/commit boundary. `activation` performs all fallible reconstruction against immutable registries first (scheduled fluids, inventory IDs, session metadata) and returns a prepared aggregate; only then does the Bevy system commit live resources and request Loading. The activation helper must receive only the definitions it actually uses, not the full scan/load registry bag. `WorldSelectionLoadContext` remains a coherent ~8-resource context rather than growing a second mega-`SystemParam`.
...
Chunk persistence must scale with authoritative edited state, not distance explored. Disk chunks use only the current state-palette plus contiguous-run encoding; obsolete per-voxel encodings are rejected rather than migrated. `WorldSnapshot` is the authoritative runtime snapshot, while `save_catalog::chunks::SavedChunkCatalog` owns its persistent chunk set. Save format 2 is the only supported disk format and never serializes that catalog inline. Each committed generation publishes persistent chunks under the matching `generation-N/` directory, writes a metadata-only `snapshot-N.json`, then publishes `manifest-N.json` last as the sole commit marker. A manifest is complete only when both metadata snapshot and external chunk generation are present. Old save formats are intentionally unsupported; persistence code must not retain migration, fallback decode, legacy serde defaults, or alternate chunk encodings. This storage protocol does not authorize gameplay-time disk writes; durable publication remains exit-only. Archived persistent chunks must serialize directly from their compact archive entries; final save must not restore a full runtime `VoxelChunk`, rebuild derived metadata/light storage, and then immediately repack the same content. `ArchivedChunk` exposes read-only occupied-content iteration, while `DiskChunk` remains the sole owner of portable block/fluid state encoding. Gameplay does not perform periodic autosaves or clock-only checkpoints. Durable world writes happen only when leaving the active world or exiting the game, including an operating-system window-close request. Those final commits remain synchronous and must complete successfully before the transition/process exit; a failed save keeps the active world/window open so the player can retry. `WorldSnapshot` is the single durable owner of day/tick state. Periodic background world publication must not be reintroduced without an explicit product/architecture decision.
...
- Save serialization ownership is split by semantics: `save_catalog::snapshot` owns current manifest/snapshot/player shapes plus capture-time intrinsic validation; `save_catalog::validation` owns registry-backed playable-content validation; `save_catalog::generations` owns published-generation discovery, current-format decode, newest→older recovery within that format, restorable selection, and retention/pruning; `save_catalog.rs` remains the public catalog facade plus create/save publication orchestration.
...
- Atomic JSON publication must sync the file contents before rename. Directory-entry syncing is platform-specific: Unix performs directory fsync after rename (and after rollback), while platforms where Rust's standard library does not expose a portable directory-sync primitive must not emulate it with `File::open(directory).sync_all()`. In particular, Windows treats that pattern as access denied. A platform-unsupported directory sync is a deliberate no-op; it must never turn an otherwise successful atomic publication into a failed save.
...
- The save reader accepts only the current save format. Manifest and snapshot format versions must both equal `SAVE_FORMAT_VERSION` and agree with each other; external chunk identity is validated again by `chunk_storage`. Unsupported historical formats are rejected rather than migrated.
- Runtime `WorldSnapshot` is deliberately not a serde type. Disk encoding/decoding is capability-owned by `DiskWorldSnapshot` and `StoredWorldSnapshot`, respectively, so core/runtime code cannot accidentally serialize the authoritative in-memory chunk catalog with metadata.
...
- Weighted biome selection carries both climate and fallback weights in one candidate record. Expensive distribution/noise eligibility must be evaluated once per candidate and reused by fallback draws. Candidate scratch should prefer inline storage for common small sets, with heap fallback only when content scale requires it.
- Geometry hot paths that only need relative distance ordering should compare squared distances and defer square roots until the winning candidate is known. Do not pay transcendental/root cost for every candidate when monotonic squared distance preserves the exact ordering.
...
`setup::progress` owns the explicit bootstrap state-machine dispatch. Its phase submodules own only the mechanics of Generation, Lighting, Meshing, and Spawning respectively; phase transitions remain visible through `WorldLoadingPhase` and must not be hidden behind callbacks/events. The shared `INITIAL_LOADING_BUDGET` remains owned by the orchestrator and phase helpers receive narrow dependencies rather than the whole setup context.
...
- Background remesh dispatch must be fair across fluid, lighting, and terrain geometry work. Continuous lighting invalidation may not starve fluid remeshes; fluid state changes must become visually observable while lighting convergence continues, and later lighting passes may schedule follow-up fluid remeshes for final illumination. A fluid remesh result is gated by voxel-content freshness, not by lighting freshness alone: if its captured fluid/block content is still current, publish the geometry even when lighting changed during the async build, then keep/queue a follow-up fluid remesh so final vertex lighting converges without making the fluid disappear or flicker. Fluid geometry reads a one-voxel 3D halo (including diagonal current/above samples for corner heights), so a fluid mutation invalidates exactly every chunk whose one-voxel mesh halo contains that voxel. Boundary-edge/corner mutations must therefore enqueue the corresponding diagonal chunk remeshes; center+cardinal invalidation is insufficient.
- Generation and runtime simulation remain separate responsibilities. Natural rivers, lakes, oceans, crater fluids, and other authored source volumes are created by deterministic generation, then their exposed dynamic frontier must fully settle before first publication. After the relevant newly generated chunk set is resident and before initial lighting/meshing, worldgen repeatedly evaluates fluid desired state with the same runtime gravity/downhill/spread rules until the local work queue reaches a true fixed point. Every successful mutation re-enqueues the dependent center/below/horizontal neighborhood; settling is resumable and frame-budgeted, never an unbounded single-frame loop. It may mutate only the newly generated chunk set and uses derived mutations that do not mark chunks persistent. Cross-boundary work whose target is outside that unpublished set remains runtime work. Bootstrap settling explicitly hands the post-settle frontier to `PendingFluidUpdates`; entering Gameplay preserves that queue and only reconciles resident frontiers. Streaming treats generated-fluid spread as a worldgen layer after a bounded generation wave, not as per-result cleanup and not as an 8×8×8 publication barrier. A wave selects at most `MAX_GENERATION_TASKS_IN_FLIGHT` new chunk targets (currently 8), lets every async generation task in that wave finish, integrates all outputs as resident/unpublished chunks, then runs `GeneratedFluidSettling` before any member becomes ready or meshable. A wave reservation is explicit ownership, not a hint: every target must always be pending for dispatch, in flight, staged, or settling-owned. If a target leaves the live selection before scheduling/result integration it is abandoned immediately; completion removes its reservation explicitly. Finishing a wave asserts that zero target reservations remain instead of clearing them blindly, so a dropped async result cannot strand a desired chunk outside every queue until the next player movement. Task completion order therefore cannot split one wave's fluid state, while wave size keeps publication latency bounded. `generation_region` remains a worldgen/cache concept, not a streaming publication unit.

Generated-fluid settling uses two correctness layers. The fast local queue propagates center/below/horizontal consequences of each mutation. Because downhill routing performs a bounded BFS, local queue exhaustion is not sufficient evidence of a fixed point: a mutation may change a farther route preference. After local work drains, settling performs a sparse verification pass over every non-source dynamic fluid cell and every currently exposed frontier target in its mutable domain. A complete verification pass with zero mutations is the only convergence condition; any mutation starts another verification round after local consequences drain. Sparse positions outside the mutable domain that can depend on the newly resident wave are retained as explicit dependency positions even when their first evaluation produces no mutation. Every verification round re-evaluates those positions and captures the content revisions of their resident chunks; therefore a later mutation inside the wave or a concurrent runtime edit cannot invalidate an already-checked external dependency without forcing another round.

The mutable settling domain starts with the unpublished generated wave and may expand through actual fluid seams into already-resident chunks only while those chunks remain eligible for derived mutation (resident and non-persistent). When a new wave becomes resident, its 3×3×3 neighboring chunk halo contributes sparse dependency candidates. Same-level neighbors contribute non-source dynamic cells and exposed frontier targets within each fluid's authored `maxSpread + 1`; chunks one level above/below contribute dynamic cells only on the shared vertical plane, again horizontally bounded by `maxSpread + 1`. Direct vertical frontier crossing remains owned by the six-cardinal boundary revisit. This lets a newly present chunk invalidate an older downhill BFS route through a vertical/diagonal seam even when no empty frontier target exists yet. `VoxelChunk` owns an incrementally maintained dynamic-fluid bitset, alongside its frontier metadata, so this halo check and fixed-point verification do not rescan all 4096 voxels per chunk. This makes seam convergence independent of generation order: old→new and new→old spread are both resolved by the generated-fluid layer. Player-authored/persistent chunks remain runtime-owned and form a hard derived-settling boundary. Existing published chunks changed by the closure receive medium-light invalidation plus fluid remesh/frontier reconciliation. Selection rebuilds exclude all wave targets, staged chunks, and settling-owned publication chunks; `mark_ready` defensively rejects every unpublished generated coord. Residency of an unpublished generated chunk is private to the generation/settling pipeline: runtime fluid due ticks targeting it must enter the existing dormant-tick path instead of mutating it, and generic chunk unloading must defer ownership until the wave publishes or archives the chunk. Residency is also not visual publication: streaming initial meshes and background remeshes may use a neighboring chunk in their one-voxel halo only when that neighbor already exists in `ChunkRenderPool`. A resident/ready/staged but not-yet-rendered neighbor is captured as absent; once it becomes visible, the existing initial-catchup/neighbor-remesh path reconciles the halo. This prevents a visible chunk from culling faces against an invisible resident chunk. This prevents runtime `set_fluid_at` from promoting staged chunks to persistent state before derived settling completes. Source cells are authoritative generated state and are never overwritten or treated as downhill destinations merely because a same-fluid source exists below an empty cell.
- Bootstrap full-field lighting relaxation must be interruptible inside voxel propagation. Loading uses the authoritative `PendingLightingUpdates` queue/context plus `process_pending_lighting()` budget slices; do not wrap an unbounded `relax()` call in an outer frame budget and call that budgeted.
