...
Runtime dynamic-fluid simulation uses one gravity-first discrete voxel solver for every fluid. Vertical descent has priority and resets `spread_distance`, so each landing starts a fresh horizontal run bounded by that fluid's authored `maxSpread`. Dynamic falling cells do not spread sideways while airborne. On supported terrain, horizontal flow searches only within its remaining `maxSpread` for the nearest reachable downward opening and prefers first-step directions on a shortest path to that opening; if no drop is reachable in-range, it falls back to ordinary radial spreading. This routing must remain fluid-ID agnostic. Generated fluid volumes from hydrology, surface-biome features, or future generators are initial fluid state rather than permanently static decoration: when a chunk becomes resident, every present fluid may seed currently empty downward/horizontal neighboring targets into the runtime solver. Do not enqueue already-filled generated fluid voxels themselves as bulk simulation work; seed only exposed empty targets, and revisit loaded neighbor boundaries when a new chunk makes cross-chunk targets available. Pending solver work is cadence-isolated by `FluidId`: generated frontiers and propagation continue in per-fluid queues, while topology edits enter a high-priority generic queue until the current world state identifies the affected fluid. A faster fluid must never demote or repeatedly recycle a slower fluid's pending work, and topology-edit priority must survive until that fluid's own `spreadSpeed` step becomes ready. Entering Gameplay rebuilds pending generated-fluid frontier work from the fully resident voxel world instead of trusting bootstrap chunk-arrival order, so initial flow must not require a player block edit to wake it. Streaming chunks still seed/revisit frontiers incrementally after Gameplay begins. A logical step freezes the active frontier for that fluid: positions created by mutations in that step are appended for the next step and may not jump to the front of the current batch. If the frame work budget ends before the frozen frontier is exhausted, the unfinished batch remains runnable on following frames without waiting for another `spreadSpeed` interval or consuming a second cadence step. Per-fluid queue pops retain their owning `FluidId`; if the current voxel state resolves a transition to another fluid, that position is rerouted to the destination fluid lane before mutation so one fluid can never execute on another fluid's cadence. Downhill routing treats an already-filled dynamic vertical column (`spread_distance == 0`) as the same reachable fall it originated from, so filling a waterfall does not make the solver forget that drop and switch directions on the next step. An empty ledge directly above the same fluid also remains a valid lower-pool destination; occupied source columns inside an existing generated pool are not misclassified as drops merely because they contain fluid below. Runtime throughput must not be improved by silently changing authored `spreadSpeed`: solver cost and queue backlog are separate concerns. Horizontal candidates should be ranked by the existing strength/tie rules before running downhill search, so weaker candidates do not pay BFS cost unnecessarily; reusable search scratch should retain allocations across updates. Generated-frontier seeding must apply the same horizontal-eligibility rule as the solver so impossible side-flow from covered unsupported source layers is never queued. Small workloads keep the normal bounded fluid budget; substantial queue backlog or accumulated step debt may use a larger bounded catch-up budget to converge without changing logical fluid cadence.
