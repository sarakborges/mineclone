...
- Generation and runtime simulation remain separate responsibilities. Natural rivers, lakes, and oceans are authored by deterministic generation rather than reconstructed by the runtime fluid solver. Generated dynamic fluid frontiers receive exactly one finite priming step before first publication: after the relevant chunk content is resident and before initial lighting/meshing, worldgen evaluates the exposed frontier targets that exist at priming time with the same runtime desired-state/downhill rules. Priming never recursively enqueues flow created by priming, never seeks a fixed point, and never bypasses authored runtime cadence beyond that first visible step. Its queue is resumable and frame-budgeted, may mutate only the newly generated chunk set, and uses derived mutations that do not mark chunks persistent. Runtime scheduling owns every continuation step. Bootstrap priming must explicitly hand off the post-prime frontier to `PendingFluidUpdates` before lighting/meshing completes; entering Gameplay must preserve that queue and may only idempotently reconcile resident frontiers, never reset the handoff. New-world generation must not enqueue pre-prime frontier wakes as authoritative runtime work. Streaming follows the same ordering by seeding runtime frontier only after its priming batch completes. Source cells are authoritative generated state and are never overwritten or treated as downhill destinations merely because a same-fluid source exists below an empty cell.
