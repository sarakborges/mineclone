...
Regional biome `avoidNear` is a symmetric separation rule between established neighboring regions, not a rarity knob. It must be evaluated against a genuinely dominant neighboring biome region, not against any single raw neighboring site draw; raw-site vetoes compound across the 8-site neighborhood and can collapse valid biome frequency into an unrestricted fallback such as Plains. Frequency remains controlled by weights/climate/distribution parameters, while `avoidNear` prevents direct adjacency to a dominant conflicting region.
