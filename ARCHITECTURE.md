...
- Generation and runtime simulation remain separate responsibilities. Natural rivers, lakes, and oceans are authored by deterministic generation rather than reconstructed by the runtime fluid solver. Generated dynamic fluid frontiers are nevertheless converged once before first publication: after the relevant chunk content is resident and before initial lighting/meshing, worldgen reuses the runtime fluid solver's desired-state/downhill rules without authored tick delays. This pre-settle is an incremental publication barrier: it owns a deduplicated work queue across frames and may spend only an explicit frame budget, so large oceans/waterfalls never block Loading or streaming in one synchronous convergence loop. It may mutate only the newly generated chunk set and must use derived mutations that do not mark chunks persistent. Targets beyond that set remain runtime work, preserving streaming/persistence ownership while ensuring a chunk does not first appear as static authored sources and only begin its initial spill after Gameplay starts.
