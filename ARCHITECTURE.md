...
Regional biome `avoidNear` is a hard symmetric proximity constraint in site selection, not a rarity knob. Do not use it merely to make two biomes less adjacent: because every candidate checks neighboring raw sites, a high-weight neighbor can suppress another regional biome across a large fraction of eligible sites. Frequency should be controlled by weights/climate/distribution parameters unless true mutual exclusion is intended.
