...
Mountain-family terrain variants are full `Surface` biomes, not child overlays and not inherited modifiers of `Mountains`. `Mountains`, `Gorge`, `Alps`, `Mountain Belt`, and `Volcano` each own their distribution, `BiomeTerrain` generator, surface layers, hydrology permissions, structures, visuals, and surface-carver permission. Macro distributions decide where a non-regional surface biome exists; the selected biome's own terrain generator decides its geometry. The local distribution strength may be passed into that generator when the geometry is inherently tied to the distribution shape, such as Gorge depth across a noise band or Volcano cone/crater shape across a peak distribution. No terrain generator may branch on a hardcoded biome ID. Volcano explicitly disables river and lake generation and uses `asteria:bassalt` as its authored surface material.
