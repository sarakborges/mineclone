...
Direct skylight is column-dependent across resident 16³ sections. Loading a section may add a new occluder above already-resident lower sections, and unloading one may reopen the sky; both transitions must explicitly invalidate/re-relax every resident section below in the same x/z chunk column. A section may never keep skylight derived from an older highest-resident-section set. The lighting work queue tracks pending voxel work per 16³ section so readiness checks are O(number of halo sections), not scans of the global queue. Interactive edits retain priority over background streaming relaxation. Asteria deliberately keeps its HSI colored voxel-light storage rather than Minecraft-style scalar/nibble light storage: the borrowed architectural idea is section readiness/dirty propagation, not its light representation.
