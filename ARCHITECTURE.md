...
Surface-carver tunnels are subordinate to cave connectivity: a surface tunnel is generated only when its carved volume intersects the anchored cave connector graph. Disconnected surface tunnels/dead ends must be rejected before density rasterization, and structure-support sampling must use the same connectivity rule. Surface-tunnel geometry must preserve a visibly curved centerline at authored lengths; do not approximate long Bezier paths with a handful of tens-of-block straight capsules. Near the terrain surface, the tunnel mouth must widen horizontally with a smooth depth-based flare while the deep profile remains circular, so an exposed tunnel does not carry a near-vertical cylindrical wall into the hillside. The mouth also owns a lake-style dry grading margin outside the carve core: terrain columns at the core boundary are graded toward the tunnel axis elevation, then the grading strength fades with smoothstep to zero at an outer normalized distance. This margin is surface-proximity gated, must be included in candidate reach/culling, and must use the same resolved geometry in terrain density and structure-support sampling.
