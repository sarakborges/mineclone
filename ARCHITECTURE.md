...
- Save serialization ownership is split by semantics: `save_catalog::snapshot` owns manifest/snapshot/player shapes plus capture-time intrinsic validation; `save_catalog::validation` owns registry-backed playable-content validation; `save_catalog::generations` owns published-generation discovery, newest→older recovery, v1/v2 decode, restorable selection, and retention/pruning; `save_catalog.rs` remains the public catalog facade plus create/save publication orchestration.
...
- Runtime `WorldSnapshot` is deliberately not a serde type. Disk encoding/decoding is capability-owned by `WorldSnapshotV2` and `StoredWorldSnapshot`, respectively, so core/runtime code cannot accidentally serialize the authoritative in-memory chunk catalog with the v2 metadata.
