...
Terrain sub-biomes are modeled as `BiomeKind::TerrainOverlay`, never as hardcoded biome-ID branches in terrain generation. A terrain overlay must name a surface `parentBiome` registered in the same dimension and uses only non-regional distributions to decide where it affects that parent. The parent remains the authoritative surface biome for visuals, hydrology, structures, creature spawning, and general biome identity; overlays modify terrain geometry through `terrainModifiers`. Multiple overlays may compose additively where their distributions overlap. An overlay may optionally define `surfaceLayers`; when present, those layers blend against the parent's material ownership according to the overlay's local distribution strength, while overlays without layers inherit the parent's materials. Positive overlay height offsets must contribute to vertical chunk bounds; negative-only cuts such as gorges must not increase those bounds. Volcano geometry is expressed by the reusable `volcanicCone` terrain modifier, whose cone/crater profile is driven by the overlay distribution strength rather than a biome-specific code path.
...
Regional biome `avoidNear` means exactly that two conflicting biome regions may not share a boundary. It is not an exclusion radius, distance check, dominant-neighbor heuristic, or rarity control. Site selection keeps the normal raw weighted biome whenever its Voronoi region does not share an edge with a conflicting raw neighbor; only a real shared Voronoi border may trigger a replacement. The rule is symmetric through `biomes_conflict`, and replacement must never reintroduce a forbidden shared border. Frequency remains controlled by weights/climate/distribution parameters.
