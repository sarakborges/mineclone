...
- Generation and runtime simulation remain separate responsibilities. Natural rivers, lakes, oceans, crater fluids, and other authored source volumes are created by deterministic generation, then their exposed dynamic frontier must fully settle before first publication. After the relevant newly generated chunk set is resident and before initial lighting/meshing, worldgen repeatedly evaluates fluid desired state with the same runtime gravity/downhill/spread rules until the local work queue reaches a true fixed point. Every successful mutation re-enqueues the dependent center/below/horizontal neighborhood; settling is resumable and frame-budgeted, never an unbounded single-frame loop. It may mutate only the newly generated chunk set and uses derived mutations that do not mark chunks persistent. Cross-boundary work whose target is outside that unpublished set remains runtime work. Bootstrap settling explicitly hands the post-settle frontier to `PendingFluidUpdates`; entering Gameplay preserves that queue and only reconciles resident frontiers. Streaming treats generated-fluid spread as a worldgen layer after coherent chunk generation, not as per-result cleanup. The finite streaming unit is the currently desired cohort inside one 8×8×8 `generation_region`: async generation tasks for that region may finish in any order, but their outputs are staged resident/unpublished until no generation task or pending desired coord remains for the active region. Only then does `GeneratedFluidSettling` run over the full newly generated cohort to a fixed point, followed by runtime-frontier/lighting seeding and first mesh publication. Chunks from another generation region wait for the active cohort to publish, preventing task completion order from deciding which neighbors existed during initial fluid spread. This gate is mandatory for every integration path that consumes freshly generated chunk output: no newly generated chunk may become ready, enter lighting/meshing publication, or become visible before its region-cohort settling completes. Restored/persisted chunks are not regenerated and therefore do not rerun worldgen settling. Selection rebuilds exclude both staged generated chunks and chunks already owned by `GeneratedFluidSettling`; the generic resident/restored fast path may never claim either state. `mark_ready` defensively rejects any unpublished generated coord. Source cells are authoritative generated state and are never overwritten or treated as downhill destinations merely because a same-fluid source exists below an empty cell.
