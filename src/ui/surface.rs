...
pub fn modal_panel() -> impl Bundle {
    frosted_surface(Node { width: px(560), padding: UiRect::all(px(28)), flex_direction: FlexDirection::Column, align_items: AlignItems::Center, row_gap: px(18), border: UiRect::all(px(1)), ..default() })
}
...
pub fn hud_control_static(selected: bool) -> (Color, Color) {
    if selected { (theme::SURFACE_INSET, HUD_SELECTED_BORDER_COLOR) } else { (theme::HUD_SURFACE, HUD_BORDER_COLOR) }
}
