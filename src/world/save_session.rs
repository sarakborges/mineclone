...
impl WorldSaveContext<'_, '_> {
    fn capture(&self, id: &str) -> io::Result<WorldSnapshot> {
        WorldSnapshot::capture(SnapshotSource {
            ...
            spawn_caves: self.state.rules.spawn_caves(),
            spawn_rivers: self.state.rules.spawn_rivers(),
            spawn_lakes: self.state.rules.spawn_lakes(),
            spawn_oceans: self.state.rules.spawn_oceans(),
