...
pub(super) fn process_dynamic_lighting(
    ...
    for coord in changed_chunks.drain() {
        ...
        // Cardinal notifications above miss edge/corner chunks, leaving their
        // already-rendered vertices with stale light even after convergence.
        // Avoid a blanket 20-chunk remesh: only a diagonal neighbor with
        // occupied boundary faces toward this chunk can use those samples.
        for y in -1..=1 {
            for z in -1..=1 {
                for x in -1..=1 {
