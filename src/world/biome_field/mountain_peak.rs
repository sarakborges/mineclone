...
pub(super) fn mountain_peak_strength(
    ...
    let seed = mix_seed(world_seed ^ string_hash(biome_id) ^ 0x517c_c1b7_2722_0a95);
    ...
    for z in -search_radius..=search_radius {
        for x in -search_radius..=search_radius {
            ...
            let jitter = Vec2::new(
                (hash_unit(hash.rotate_left(17)) * 2.0 - 1.0) * jitter_extent,
                (hash_unit(hash.rotate_left(37)) * 2.0 - 1.0) * jitter_extent,
            );
...
fn fractal_noise(position: Vec2, seed: u64) -> f32 {
    ...
    for octave in 0..PEAK_NOISE_OCTAVES {
