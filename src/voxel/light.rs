...
impl VoxelLight {
    ...
    pub const fn new_colored(sky: [u8; 3], block: [u8; 3]) -> Self {
        let sky_r = clamp_channel(sky[0]) as u32;
        let sky_g = clamp_channel(sky[1]) as u32;
        let sky_b = clamp_channel(sky[2]) as u32;
        let block_r = clamp_channel(block[0]) as u32;
        let block_g = clamp_channel(block[1]) as u32;
        let block_b = clamp_channel(block[2]) as u32;
...
const fn max_channel(value: [u8; 3]) -> u8 {
    let first = if value[0] > value[1] { value[0] } else { value[1] };
...
mod tests {
    ...
    #[test]
    fn packs_colored_channels() {
        let light = VoxelLight::new_colored([15, 6, 1], [2, 9, 14]);

        assert_eq!(light.sky_rgb(), [15, 6, 1]);
    ...
    fn clamps_channels_to_four_bits() {
        ...
        assert_eq!(light.sky_rgb(), [15, 1, 15]);
