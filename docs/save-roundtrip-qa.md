...
| --- | --- | --- |
| Identity | Create two worlds with the same requested name, using different seeds; open each again. Also try `COM¹`, `LPT²` and `COM³.txt` and valid Unicode names close to the 200 UTF-16-unit limit. | Second world has a unique name; neither world overwrites the other's directory or data. Windows device names are rejected before attempting to create a directory. |
